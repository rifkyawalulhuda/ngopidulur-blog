...
@if (! $hasQuery)
    <div class="rounded-[1.45rem] border border-[#efddcc] bg-white/96 p-4 text-sm text-[#7c695d] shadow-[0_24px_55px_-40px_rgba(90,46,22,0.42)] backdrop-blur-sm dark:border-coffee-700/40 dark:bg-neutralwarm-900/96 dark:text-neutralwarm-100/70">
...
@else
    <div class="overflow-hidden rounded-[1.6rem] border border-[#efddcc] bg-white/96 shadow-[0_28px_60px_-42px_rgba(90,46,22,0.5)] backdrop-blur-sm dark:border-coffee-700/40 dark:bg-neutralwarm-900/96">
        ...
        <div class="divide-y divide-[#f3e6da] dark:divide-white/8">
            @foreach ($posts as $post)
                <a href="{{ route('posts.show', $post->slug) }}" class="group flex items-start gap-3 px-4 py-3 transition hover:bg-[#fcf5ee] dark:hover:bg-white/5">
                    <span class="mt-1 inline-flex size-10 shrink-0 items-center justify-center rounded-2xl bg-[#fbf2e9] text-[#8b4a22] dark:bg-white/6 dark:text-coffee-100">
                    ...
                    <div class="min-w-0 flex-1">
                        <div class="flex flex-wrap items-center gap-2 text-[11px] font-semibold uppercase tracking-[0.18em] text-[#9b8476] dark:text-neutralwarm-100/55">
                            <span class="rounded-full bg-[#f7ebdf] px-2.5 py-1 text-[#8b4a22] dark:bg-coffee-500/18 dark:text-coffee-100">
                                {{ $post->category?->name ?? 'Artikel' }}
                            </span>
                            <span>{{ $post->published_at?->translatedFormat('d M Y') }}</span>
                        </div>

                        <p class="mt-2 line-clamp-2 font-lora text-lg font-semibold leading-tight text-[#2f1c12] transition group-hover:text-[#8b4a22] dark:text-neutralwarm-50 dark:group-hover:text-coffee-100">
                            {{ $post->title }}
                        </p>
        ...
        <div class="border-t border-[#f1e3d6] px-4 py-3 dark:border-white/8">
